...
def BMI_page(root, main_menu):
    ...
    title = tk.Label(card, text="BMI Dashboard",
                     font=("Segoe UI", 26, "bold"), bg="white")
    ...
    weight_entry = tk.Entry(card, font=("Segoe UI", 16),
                            width=12, bd=2, relief="groove")
    weight_entry.grid(row=1, column=1, pady=10)

    tk.Label(card, text="Height (cm or m):",
             font=("Segoe UI", 16), bg="white")\
        .grid(row=2, column=0, sticky="e", padx=15, pady=10)

    height_entry = tk.Entry(card, font=("Segoe UI", 16),
                            width=12, bd=2, relief="groove")
    height_entry.grid(row=2, column=1, pady=10)

    tk.Label(card, text="Activity Level:",
             font=("Segoe UI", 16), bg="white")\
        .grid(row=3, column=0, sticky="e", padx=15, pady=10)

    activity_var = tk.StringVar()
    activity_combo = ttk.Combobox(
        card, textvariable=activity_var,
        font=("Segoe UI", 14), width=14,
        values=[
    ...
    gauge_canvas = tk.Canvas(card, width=300, height=180,
                             bg="white", highlightthickness=0)
    gauge_canvas.grid(row=5, column=0, columnspan=2, pady=20)

    result_label = tk.Label(card, text="",
                            font=("Segoe UI", 18), bg="white")
    result_label.grid(row=6, column=0, columnspan=2, pady=10)

    bar_canvas = tk.Canvas(card, width=300, height=25,
                           bg="white", highlightthickness=0)
    ...
    bar_pointer = bar_canvas.create_line(0, 0, 0, 25,
                                         width=4, fill="black")
    ...
    def calculate():
        try:
            ...
            if "Sedentary" in level:
                mult = 1.2
            elif "Lightly" in level:
                mult = 1.375
            elif "Moderately" in level:
                mult = 1.55
            elif "Very" in level:
                mult = 1.725
            else:
            ...
            gauge_canvas.delete("all")
            bar_canvas.coords(bar_pointer, 0, 0, 0, 25)

    tk.Button(card, text="Calculate",
              font=("Segoe UI", 16),
              bg="#4da6ff", fg="white",
              padx=20, pady=5,
              command=calculate)\
    ...
    tk.Button(card, text="🔙 Back to Main Menu",
              font=("Segoe UI", 14),
              bg="#e0e0e0", fg="black",
              padx=20, pady=5,
              command=main_menu)\
